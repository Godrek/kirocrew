...
| H6 | A capability is granted by opt-in membership, never by negation. `is_session_sharing_eligible` reads `ACP_BACKENDS_SESSION_SHARING`, `supports_steer` reads `ACP_BACKENDS_STEER`, and the model-selection capabilities read `ACP_BACKENDS_KIRO_MODEL_CATALOG` / `ACP_BACKENDS_REGISTRY_MODEL_CATALOG` / `ACP_BACKENDS_SET_MODEL_METHOD` / `ACP_BACKENDS_MODEL_CONFIG_OPTION` / `ACP_BACKENDS_REASONING_EFFORT` — so a harness that has not demonstrated the capability does not inherit it from a set it was never added to. | `test_harness_parity.py::test_session_sharing_is_opt_in`, `::test_steer_is_opt_in`, `test_backend_model_capabilities.py::TestCapabilityResolution` | `providers/acp.py` (`AcpProvider.is_session_sharing_eligible`, `.supports_model_switch`), `acp/client.py` (`AcpClient.supports_steer`), `acp/model_catalog.py`, `acp/types.py` |
| H9 | A harness's model vocabulary is its OWN. Discovery is scoped to live sessions of the same backend (`_providers_for_backend`), the catalog source is resolved from membership rather than backend identity, and a harness in neither catalog set shows only what its own session advertised — never another harness's list and never an invented id. Its default model lives in its own config namespace (`agent.backend_models.<backend>`, with the kiro family keeping `agent.model`), so switching `acp_backend` cannot overwrite or reinterpret another harness's saved pick. Reasoning effort is likewise a harness property first and a model property second. | `test_backend_model_capabilities.py::TestAdvertisedScoping`, `::TestModelsEndpoint`, `::TestBackendModelConfig` | `acp/model_catalog.py`, `dashboard/handlers/agents.py` (`_advertised_for_backend`, `api_models`, `api_model_capabilities`), `config/loader.py` (`AgentConfig.model_for_backend`) |
